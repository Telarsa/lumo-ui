 ...
 */
export function isLatinRun(text: string): boolean {
  return !NON_LATIN_LETTER.test(text);
}

/** The attribute the gate honours, as an object to spread. */
export type LatnAttrs = { "data-lumo-latn"?: "" };
 ...
 */
export function latnAttrs(text: string): LatnAttrs {
 ...
 */
export function latnNodeAttrs(chunks: ReactNode): LatnAttrs {
  const text =
