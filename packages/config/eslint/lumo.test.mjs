...
describe("the policy is wired to something that actually runs", () => {
  ...
  it("lints the two trees the contract is about", async () => {
    // A config that resolves but matches no component file is the vacuous pass
    // `packages/gate/src/cli.ts` refuses loudly for HTML. Same refusal here.
    const repoEslint = new ESLint({ cwd: REPO });
    for (const dir of ["packages/ui/src", "packages/blocks/src"]) {
      const results = await repoEslint.lintFiles([join(REPO, dir)]);
      expect(results.length, `${dir} contributed no linted files`).toBeGreaterThan(20);
    }
  });
});
