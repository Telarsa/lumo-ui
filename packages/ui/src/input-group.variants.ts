...
/**
 * In a `.variants.ts` companion, not `input-group.tsx`: a `cva()` exported
 * from a `"use client"` module is a client reference a server component cannot call.
 */
...
export const inputGroupInputVariants = cva(
    ...
    "data-disabled:cursor-not-allowed data-disabled:bg-surface-sunken",
  {
    variants: {
      ...
      // Padding is a variant of the slot's PRESENCE, so an adornment-free group matches a TextField.
