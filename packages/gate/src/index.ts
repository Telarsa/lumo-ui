...
import { LATN_HATCH, RULES, digitSystem, scriptSystem, type DigitSystem, type Doc, type Rule, type ScriptSystem, type Violation } from "./rules.ts";
 ...
 */
function directionFor(tag: string, primary: string): "rtl" | "ltr" {
  try {
...
/** Per-run settings the CLI reads from the floors file and hands to every grader. */
export interface GradeOptions {
  /** `@locales` — see `localeForPath`. */
...
export function localeForPath(
  ...
  if (declared !== undefined) {
    ...
    if (hit !== undefined) {
      ...
      if (known !== undefined) return { locale: known, direction: gradingFor(known).direction };
    ...
    throw new Error(
      `Cannot derive a locale from route ${JSON.stringify(path)}: no segment is one of the declared locales ` +
...
function isLanguageTag(tag: string): boolean {
  try {
    if (Intl.getCanonicalLocales(tag).length !== 1) return false;
...
export function gradeHtml(path: string, html: string, rules: Rule[] = RULES, options: GradeOptions = {}): Violation[] {
  ...
  for (const el of Array.from(document.querySelectorAll('[hidden]'))) {
    if (/^S:\d/.test(el.getAttribute("id") ?? "")) el.removeAttribute("hidden");
  }
 ...
 */
export function addCoverage(into: Coverage, path: string, html: string, options: GradeOptions = {}): Coverage {
  const { document } = parseHTML(html);
  const { locale } = localeForPath(path, undefined, (document as unknown as Document).documentElement?.getAttribute("lang") ?? undefined, options.declaredLocales);
...
export function countNativeDigits(path: string, html: string, options: GradeOptions = {}): number {
  const { document } = parseHTML(html);
  const { locale } = localeForPath(path, undefined, (document as unknown as Document).documentElement?.getAttribute("lang") ?? undefined, options.declaredLocales);
  ...
  for (let node = walker.nextNode(); node !== null; node = walker.nextNode()) {
    ...
    const tag = parent?.tagName?.toLowerCase();
    if (tag === "script" || tag === "style" || parent?.closest?.(LATN_HATCH)) continue;
 ...
 */
export function exceedsExemptCeiling(
  c: Coverage,
  ceiling: number | undefined,
): { fraction: number; ceiling: number } | undefined {
  if (ceiling === undefined || c.characters === 0) return undefined;
