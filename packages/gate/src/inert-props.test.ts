 ...
 * an audit worded the task, would have missed most of what it found:
...
`;
  ...
  it("ignores props inherited from another module", () => {
    const src = `
import type { DOMProps } from "../../core/src/index.ts";
...
`;
  ...
  it("grades checker-resolved inherited behavior supplied by the CLI", () => {
    const src = `
import type { PressEvents } from "../../core/src/index.ts";
...
describe("the gate is wired into verify", () => {
  ...
  /*
   * The inert-props tier is a TOOL, not a gate, since §50.6.
   *
   * These three tests asserted its wiring into `verify` — that `gate:props`
   * ran, ran before `gate:test`, and graded `packages/ui/src` and
   * `packages/blocks/src`. All three packages were deleted with the component
   * library, the script was removed with them, and the assertions had been
   * failing ever since: `verify` was red on this file and on four others for
   * five commits, because the failure was reported through a shell pipeline
   * whose exit code was the tail's, not the run's.
   *
   * What replaces them is the honest claim: the tool still ships, still has
   * its fixtures, and is documented as something you point at a consumer's
   * owned copies (CONTRIBUTING.md). If it is ever wired into `verify` again,
   * the assertion belongs here — pointed at whatever tree it then grades.
   */
  it("ships as a runnable CLI with its fixtures, even though no gate runs it", () => {
    const cli = join(import.meta.dirname, "inert-props-cli.ts");
    expect(existsSync(cli), "the inert-props CLI is gone").toBe(true);

    const fixtures = join(import.meta.dirname, "..", "fixtures", "inert-props");
    expect(existsSync(join(fixtures, "good.tsx")), "the clean fixture is gone").toBe(true);

    // And no script claims to run it, which is what makes the line above true.
    const verify = root.scripts["verify"] ?? "";
    expect(verify).not.toContain("gate:props");
    expect(root.scripts["gate:props"]).toBeUndefined();
  });
