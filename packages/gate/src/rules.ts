...
/**
 * The deliberately-foreign hatch. `data-lumo-latn` is the author's mark; the
 * four phrasing elements whose CONTENT is machine text by definition — `<code>`,
 * `<kbd>`, `<samp>`, `<var>` — carry the same meaning by their semantics, so a
 * page does not have to mark «<code>WEBVTT</code>» to say what `<code>` already
 * says. `<pre>` is NOT here: preformatted is a layout fact (poetry, addresses),
 * not a language one; `<pre><code>` is covered by the inner element. `lang`
 * is deliberately NOT here either — it is the natural wrong "fix" for text in
 * the wrong script, and the gate refuses to honour it.
 */
export const LATN_HATCH = "[data-lumo-latn],code,kbd,samp,var";
...
const INTERACTIVE =
  ...
  // Roles ARIA requires a name on that this list had simply never included.
  // Checked against axe-core's own `accessibleNameRequired` table rather than
  // from memory. `progressbar` is the one with a live instance: a consumer build
  // serves `<div role="progressbar" aria-valuetext="۲۵٪">` with no name, so a
  // reader is told «۲۵٪» and never what is at 25%. Base UI emits all of these —
  // MenuCheckboxItem, MenuRadioItem and ProgressRoot each set their role — so
  // any consumer on it inherits the gap.
  //
  // `columnheader` and `rowheader` are deliberately NOT here: axe marks them
  // accessibleNameRequired FALSE, and a table header is named by its content.
  '[role=treeitem],[role=menuitemcheckbox],[role=menuitemradio],' +
  '[role=progressbar],[role=toolbar],' +
  ...
  const walk = (n: Node) => {
    for (const child of Array.from(n.childNodes)) {
      if (child.nodeType === 3) {
        ...
        if (el.closest?.(LATN_HATCH)) continue;
        walk(el);
      }
    }
  };
...
function visibleTextRuns(doc: Document): TextRun[] {
  ...
  const walk = (el: Element | Document) => {
    ...
    for (const child of Array.from(el.childNodes)) {
      if (child.nodeType === 3) {
        ...
        if (kid.closest?.(LATN_HATCH)) continue;
        walk(kid);
      }
    }
  ...
  if ((body as Element).closest?.(LATN_HATCH)) return out;
  walk(body);
  return out;
}
...
export const langDir: Rule = {
  ...
  run: (doc) => {
    ...
    /*
     * The comparison is on the PRIMARY SUBTAG, not the whole tag.
     *
     * `lang` picks the speech synthesiser, and a synthesiser is chosen by
     * LANGUAGE: `lang="en"` on an `en-US` route is less specific, not wrong,
     * and a site that legitimately serves plain `en` was failing this rule for
     * saying so (found wiring a consumer's site, 31 Aug 2026).
     *
     * The founding defect is untouched, because it was a different LANGUAGE:
     * `lang="en"` on a Persian page hands 187 Persian names to an English
     * voice, and `en` ≠ `fa` still fails here. What no longer fails is a page
     * that names its own language correctly with fewer subtags.
     *
     * Region and script are NOT ignored generally — they decide digits and
     * calendars, and `no-latin-digits` and `native-calendar` grade those from
     * the ROUTE's locale, which keeps its full precision.
     */
    const langPrimary = lang === null ? null : lang.toLowerCase().split("-")[0];
    const expectedPrimary = doc.locale.toLowerCase().split("-")[0];
    if (langPrimary !== expectedPrimary) {
...
/**
 * Does this text contain a LETTER of the reader's script?
 *
 * Not "a character" — the distinction is the whole point. Persian and
 * Arabic-Indic digits carry `Script=Arabic`, so `/\p{Script=Arabic}/u.test("۲")`
 * is TRUE, and the three language rules below each read that as "the reader's
 * script is present here, a translator has been over this string". A localised
 * digit is no evidence of that whatsoever.
 *
 * The effect was a hole in exactly the state these rules exist to catch —
 * half-translated UI. «Page ۲ of ۱۰», «Next page ۳», «Showing ۱ – ۲۰ of ۳۴۷
 * results»: someone ran the numbers through `formatNumber` and left the words
 * in English, and all thirteen rules said nothing.
 *
 * `latn-island-purity` has always got this right — it builds
 * `/(?=\p{L})\p{Script=Latin}/gu` for the same reason — so this is the rest of
 * the file catching up with one rule in it.
 */
function hasReaderLetter(text: string, script: { pattern: RegExp }): boolean {
  return new RegExp(`(?=\\p{L})${script.pattern.source}`, "u").test(text);
}
  ...
  run: (doc) => {
    ...
    for (const { attr, on } of SPOKEN) {
      ...
      for (const el of Array.from(doc.document.querySelectorAll(selector))) {
        if (el.closest?.(LATN_HATCH)) continue;
        // The same skip the other eight rules apply, added 31 Aug 2026. This
        // rule skipped NEITHER, which made it the strictest of the thirteen for
        // no stated reason; §50.5 recorded the divergence and left it open.
        //
        // The other direction was measured before settling this one: grading
        // `[hidden]` everywhere — on the argument that a closed dialog SHIPS its
        // strings and a click announces them — produced ZERO new violations
        // across 114 documents (the docs site and both example apps). Zero
        // evidence is not a reason to reverse §50.5's tested position that
        // genuinely hidden UI is not graded as if a reader could reach it. The
        // React-streaming case that position was carved for is handled where it
        // belongs, in `gradeHtml`, which un-hides `S:` containers before any
        // rule runs.
        if (el.closest?.('[aria-hidden="true"],[hidden]')) continue;
        ...
        if (LATIN_WORD.test(value) && !hasReaderLetter(value, doc.script)) {
...
/**
 * The elements that carry those item roles IMPLICITLY, per container.
 *
 * Three of the item roles above are ones nobody ever writes: a `<button>` is a
 * `button`, a `<td>` inside a grid is a `gridcell`, a `<tr>` is a `row`. The
 * selector was built purely from `[role="…"]`, so for canonical markup those
 * three containers matched no items at all — and `if (items.length === 0)
 * continue` then made the rule pass in silence. A real Radix Toolbar of three
 * `<button tabindex="-1">`, with zero keyboard entry points, graded clean.
 *
 * That is the failure this file's own header forbids: "each is designed so it
 * CANNOT pass vacuously". The ceiling rule next door already saw those buttons,
 * because it asks `isTabbable` rather than matching a role — so the floor and
 * the ceiling disagreed about the same set of elements.
 *
 * `:not([role])` because an explicit role wins over the implicit one, and the
 * selector is evaluated INSIDE the container element, which is what makes
 * `td` mean `gridcell` here rather than the bare `cell` it means elsewhere.
 */
const IMPLICIT_ITEM_TAGS: Record<string, readonly string[]> = {
  toolbar: ["button"],
  grid: ["td", "th"],
  treegrid: ["tr"],
};
...
export const compositeTabStop: Rule = {
  ...
  run: (doc) => {
    ...
    for (const [containerRole, itemRoleSpec] of Object.entries(COMPOSITE_ROLES)) {
      ...
      const itemSelector = [
        ...itemRoles.map((r) => `[role="${r}"]`),
        ...(IMPLICIT_ITEM_TAGS[containerRole] ?? []).map((tag) => `${tag}:not([role])`),
      ].join(",");
...
/**
 * Month names a different formatter produces for the same calendar.
 *
 * `Intl` is not the only thing that renders a Gregorian month in a non-Latin
 * script. date-fns — which is what `react-day-picker` uses, and therefore what
 * shadcn's Calendar uses — ships its own transliterations, and they disagree
 * with CLDR on five of twelve Persian months. A rule built only from `Intl`
 * cannot see a date produced by the most likely library in the stack.
 *
 * Captured from `react-day-picker@10.0.1/locale`'s `faIR.localize.month(i,
 * {width:"wide"})` rather than typed from memory.
 */
const TRANSLITERATED_MONTHS: Record<string, readonly string[]> = {
  fa: [
    "ژانویه", "فوریه", "مارس", "آپریل", "می", "جون",
...
function monthNames(locale: string, calendar: string): string[] {
  ...
  if (!names) {
    ...
    // THREE SOURCES, because one was never enough and the rule was silently
    // missing most real dates.
    //
    // 1. The STANDALONE form (`{month:"long"}`). This is all the rule used to
    //    have — and no page renders a month standalone. A page renders a DATE.
    // 2. The FORMAT form, lifted out of a real formatted date. Persian takes
    //    the ezafe on four months, so `{dateStyle:"long"}` on 22 July gives
    //    «۲۲ ژوئیهٔ ۲۰۲۴» — «ژوئیه» plus U+0654, which is `\p{M}`, which
    //    `datePattern`'s trailing boundary then refused to match. Four of
    //    twelve months evaded the rule entirely: January, February, May, July.
    // 3. The date-fns / react-day-picker transliterations, as a literal list.
    //    A different tool made different choices — «جولای» not «ژوئیه»,
    //    «آپریل» not «آوریل» — and five of twelve names were unmatchable.
    //
    // July is in BOTH missing sets, and «۲۲ ژوئیه ۲۰۲۴» is the string in this
    // rule's own `because` and in its poison fixture, which claims to be
    // "exactly what react-day-picker's v10 locale/fa-IR produces". It is not:
    // that library says «جولای». The fixture was written beside the
    // implementation, in the one form the implementation could already see.
    // Mid-month, so a timezone slip cannot roll into a neighbouring month.
    const dated = new Intl.DateTimeFormat(`${locale}-u-ca-${calendar}`, {
      dateStyle: "long",
      timeZone: "UTC",
    });
    const standalone = Array.from({ length: 12 }, (_, i) =>
      format.format(new Date(Date.UTC(2024, i, 15))),
    );
    // The month as it appears INSIDE a date: the longest run of letters and
    // combining marks, which is the month name however the locale inflects it.
    const inDates = Array.from({ length: 12 }, (_, i) => {
      const rendered = dated.format(new Date(Date.UTC(2024, i, 15)));
      const runs = rendered.match(/[\p{L}\p{M}]+/gu) ?? [];
    ...
    names = [
      ...new Set([
        ...standalone,
        ...inDates,
        // ONLY for the calendar they transliterate. The list is Gregorian
        // month names in Persian script; folding it into the NATIVE (Jalali)
        // list too made `foreign.filter(n => !native.includes(n))` delete every
        // one of them, and the rule went from missing four months to missing
        // all twelve. Caught by re-measuring instead of assuming.
        ...(calendar === "gregory" ? TRANSLITERATED_MONTHS[locale.split("-")[0] ?? ""] ?? [] : []),
      ]),
    ];
...
function datePattern(monthName: string): RegExp {
  /*
   * ZWNJ IS PART OF THE WORD.
   *
   * «می» is the Persian name for May and also the commonest verb prefix in the
   * language, and the two are told apart by what follows. `می‌افتد` ("falls")
   * is ONE word joined by U+200C; `می ۲۰۲۴` is a date. U+200C is a format
   * character, not a letter or a mark, so a boundary built from `\p{L}\p{M}`
   * alone treats `می‌افتد` as the month name followed by a word break — and
   * «۱ می‌افتد» ("1 falls") gets reported as "1 May".
   *
   * `persian-zwnj` exists because «می کند» and «می‌کند» are different strings.
   * This rule has to agree: the joiner is part of the word on both sides.
   */
  const L = "\\p{L}\\p{M}\\u200C\\u200D";
  ...
  run: (doc) => {
    ...
    if (doc.calendar === "persian" || doc.calendar.startsWith("islamic")) {
      for (const seg of Array.from(doc.document.querySelectorAll('[role="spinbutton"][data-type="year"][aria-valuenow]'))) {
        if (seg.closest?.(`[aria-hidden="true"],[hidden],${LATN_HATCH}`)) continue;
...
export const nativeScriptText: Rule = {
  ...
  run: (doc) => {
    ...
    for (const { element, text } of visibleTextRuns(doc.document)) {
      if (hasReaderLetter(text, doc.script)) continue;
      if (!FOREIGN_WORD.test(text)) continue;
      v.push({
        rule: "native-script-text",
        path: doc.path,
        detail:
          `visible text with no ${doc.script.name} letter at all: ` +
...
function foreignResidue(el: Element, name: string): string {
  let residue = name;
  for (const marked of Array.from(el.querySelectorAll(LATN_HATCH))) {
...
export const nativeScriptName: Rule = {
  ...
  run: (doc) => {
    ...
    for (const el of Array.from(doc.document.querySelectorAll(INTERACTIVE))) {
      ...
      if (!name) continue;
      if (hasReaderLetter(name, doc.script)) continue;
      ...
      v.push({
        ...
        detail:
          ...
          `which holds no ${doc.script.name} letter. That is the string a ${doc.locale} ` +
...
export const latnIslandPurity: Rule = {
  ...
  run: (doc) => {
    ...
    // Purity audits the AUTHOR'S claim — `data-lumo-latn` says "this is Latin".
    // `<code>` claims nothing about script: a sample of Persian text inside it
    // is machine text in Persian, which is exactly what `<code>` is for. 0.5.6
    // iterated the whole hatch here and failed 52 Persian code samples.
    for (const island of Array.from(doc.document.querySelectorAll("[data-lumo-latn]"))) {
      if (island.parentElement?.closest(LATN_HATCH)) continue;
...
/**
 * Rule 14 — the Persian imperfective prefix written with a SPACE where the
 * language uses a zero-width non-joiner (U+200C, «نیم‌فاصله»).
 *
 * «می‌کند» is one word. «می کند» is two, and every layer below the glyphs
 * treats it that way:
 *
 *   - `Intl.Collator("fa-IR")` reports the two spellings UNEQUAL at every
 *     sensitivity, so a sorted list splits them.
 *   - substring search fails in BOTH directions: a reader who types the
 *     correct form finds nothing, and vice versa.
 *   - it is an intra-word line-break opportunity, so a narrow column may
 *     split the verb. (Font- and width-dependent, unlike the two above,
 *     which are unconditional — the rule does not rest on it.)
 *
 * FROM THE FIELD: a live six-language product serves 29 of these across its 11
 * Persian routes, INTERLEAVED with 96 correct joins in the same files — one
 * document has 17 broken against 5 correct. That is inconsistent authoring, not
 * a house style, which is exactly the shape a grader is for and a style guide
 * is not.
 *
 * DELIBERATELY NARROW. Only the verbal prefix `می`/`نمی`, and only immediately
 * before an Arabic-script letter. The comparative suffix `تر`/`ترین` has the
 * same orthography and was left out: «چوب تر» (wet wood), «نان تر» and «میوه
 * تر» are correct Persian and would fire. Telling those apart needs a lexicon,
 * and a rule that cries wolf on correct prose is a rule people switch off.
 *
 * Residual risk, stated rather than hidden: `می` is also the noun "wine", so
 * «می ناب» — a literary construction essentially absent from product UI —
 * fires. Measured against real content it did not occur: all 11 distinct
 * matches in the worst document were genuine verbs.
 */
export const persianZwnj: Rule = {
  id: "persian-zwnj",
  because:
    "«می کند» and «می‌کند» are different strings to every collator and every " +
    "search box. A live product serves 29 of the broken form beside 96 of the " +
    "correct one, in the same files.",
  run: (doc) => {
    if (!doc.script.pattern.test("ک")) return []; // Arabic-script locales only
    if (doc.locale.split("-")[0]?.toLowerCase() !== "fa") return [];
    const broken = /(?<![\p{L}\p{M}])(ن?می) (?=[\u0600-\u06FF])/u;
    const v: Violation[] = [];
    for (const node of visibleTextNodes(doc.document)) {
      if (node.parentElement?.closest?.(LATN_HATCH)) continue;
      const match = broken.exec(node.data);
      if (match === null) continue;
      const at = node.data.slice(match.index, match.index + 24).trim();
      v.push({
        rule: "persian-zwnj",
        path: doc.path,
        detail:
          `«${match[1] ?? "می"}» is joined with a SPACE, not a zero-width non-joiner: ` +
          `${JSON.stringify(at)}. Two words to every collator and every search box; ` +
          `write «${match[1] ?? "می"}\u200C».`,
        snippet: node.parentElement?.outerHTML?.slice(0, 120),
      });
    }
    return v;
  },
...
export const RULES: Rule[] = [langDir, noLatinDigits, noLatinAria, namedControls, resolvedIdrefs, compositeTabStop, compositeSingleTabStop, nativeCalendar, uniqueIds, nativeScriptText, nativeScriptName, namedRoledescription, latnIslandPurity, persianZwnj];
