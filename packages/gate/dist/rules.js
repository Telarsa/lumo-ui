 ...
 */
function hasReaderLetter(text, script) {
...
function monthNames(locale, calendar) {
    ...
    if (!names) {
        ...
        const standalone = Array.from({ length: 12 }, (_, i) => format.format(new Date(Date.UTC(2024, i, 15))));
    ...
    run: (doc) => {
        ...
        if (doc.calendar === "persian" || doc.calendar.startsWith("islamic")) {
            for (const seg of Array.from(doc.document.querySelectorAll('[role="spinbutton"][data-type="year"][aria-valuenow]'))) {
                if (seg.closest?.(`[aria-hidden="true"],[hidden],${LATN_HATCH}`))
                    continue;
...
export const nativeScriptText = {
    ...
    run: (doc) => {
        ...
        for (const { element, text } of visibleTextRuns(doc.document)) {
            if (hasReaderLetter(text, doc.script))
                continue;
            ...
            v.push({
                ...
                path: doc.path,
                detail: `visible text with no ${doc.script.name} letter at all: ` +
...
export const nativeScriptName = {
    ...
    run: (doc) => {
        ...
        for (const el of Array.from(doc.document.querySelectorAll(INTERACTIVE))) {
            ...
            if (!name)
                continue;
            if (hasReaderLetter(name, doc.script))
                continue;
...
export const latnIslandPurity = {
    ...
    run: (doc) => {
        ...
        for (const island of Array.from(doc.document.querySelectorAll("[data-lumo-latn]"))) {
            if (island.parentElement?.closest(LATN_HATCH))
                continue;
 ...
 */
export const persianZwnj = {
    id: "persian-zwnj",
    because: "«می کند» and «می‌کند» are different strings to every collator and every " +
        "search box. A live product serves 29 of the broken form beside 96 of the " +
        "correct one, in the same files.",
    run: (doc) => {
        if (!doc.script.pattern.test("ک"))
            return []; // Arabic-script locales only
        if (doc.locale.split("-")[0]?.toLowerCase() !== "fa")
            return [];
        const broken = /(?<![\p{L}\p{M}])(ن?می) (?=[\u0600-\u06FF])/u;
        const v = [];
        for (const node of visibleTextNodes(doc.document)) {
            if (node.parentElement?.closest?.(LATN_HATCH))
                continue;
            ...
            v.push({
                rule: "persian-zwnj",
                path: doc.path,
                detail: `«${match[1] ?? "می"}» is joined with a SPACE, not a zero-width non-joiner: ` +
...
export const RULES = [langDir, noLatinDigits, noLatinAria, namedControls, resolvedIdrefs, compositeTabStop, compositeSingleTabStop, nativeCalendar, uniqueIds, nativeScriptText, nativeScriptName, namedRoledescription, latnIslandPurity, persianZwnj];
