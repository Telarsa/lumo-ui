...
import { addCoverage, EMPTY_COVERAGE, exceedsExemptCeiling, format, formatCoverage, gradeHtml, missingDenseDigitFloors, } from "./index.js";
...
async function readFloors(path) {
    ...
    return {
        floors: Object.fromEntries(Object.entries(raw).filter(([k, v]) => !k.startsWith("//") && !k.startsWith("@") && typeof v === "number")),
        exemptCeiling: typeof ceiling === "number" ? ceiling : undefined,
        minDocuments: typeof minDocuments === "number" ? minDocuments : undefined,
        declaredLocales: Array.isArray(locales) ? new Set(locales.filter((l) => typeof l === "string")) : undefined,
        settings,
    };
}
const rules = [...RULES];
 ...
 */
const EMBEDDED_ASSET_DIRS = new Set([]);
...
if (graded.size === 0) {
    ...
    console.error(`  ${String(files.length)} file(s) were found and every one was skipped as a redirect.`);
    console.error("  Refusing to report success on nothing.");
    process.exit(2);
}
/*
...
if (typeof minDocuments === "number" && graded.size < minDocuments) {
    console.error(`  lumo-gate graded ${String(graded.size)} document(s) under ${root}, ` +
        `below the committed floor of ${String(minDocuments)}.`);
    console.error("  A build that emits fewer pages than it used to has not got cleaner — it has got smaller.");
    console.error("  If the drop is deliberate, lower @min-documents in the floors file in the same commit.");
    process.exit(2);
}
...
if (over) {
    violations.push({
        rule: "exempt-ceiling",
        path: "(whole build)",
        detail: `${over.fraction.toFixed(1)}% of characters in non-latn documents are exempt via ` +
...
const summary = distinct === violations.length
