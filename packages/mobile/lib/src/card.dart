...
class _LumoCardState extends State<LumoCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final decoration = BoxDecoration(
      ...
      boxShadow: widget.variant == LumoCardVariant.elevated ? [BoxShadow(color: c.scrim.withValues(alpha: 0.12), blurRadius: 8, offset: const Offset(0, 2))] : null,
    );
    final body = Padding(padding: widget.padding, child: DefaultTextStyle.merge(style: TextStyle(color: c.fg), child: widget.child));
    ...
    return Semantics(
      ...
      child: Opacity(
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(duration: const Duration(milliseconds: 80), decoration: decoration, child: body),
        ),
      ),
    );
  }
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Semantics(header: true, child: Text(title, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, height: 1.4, color: c.fg))),
                if (description != null) Padding(padding: const EdgeInsets.only(top: 4), child: Text(description!, style: TextStyle(fontSize: 14, color: c.fgMuted))),
              ],
            ),
          ),
