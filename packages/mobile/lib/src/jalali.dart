...
enum LumoDateStyle {
  ...
  long,
  /// Day and month, NO year: «۲۶ مرداد» / `August 17`.
  ///
  /// For dates already understood to be recent — a diary header, a log entry,
  /// anything inside a view of "this month". The year is the noisiest part of
  /// a date and the part the reader almost always already knows.
  ///
  /// The web has always been able to express this (`formatDate(d, locale,
  /// {day: 'numeric', month: 'long'})`, since it forwards
  /// `Intl.DateTimeFormatOptions` wholesale); this enum is a narrowing of that
  /// contract and simply had a hole in it. Consumers were composing the string
  /// by hand from an English month-name array, which is exactly the code that
  /// does not survive contact with Persian.
  dayMonth,
}
...
String formatLumoDate(DateTime date, String locale, {LumoDateStyle style = LumoDateStyle.medium}) {
  if (calendarOf(locale) == LumoCalendarSystem.gregorian) {
    ...
    switch (style) {
      ...
      case LumoDateStyle.dayMonth:
        return DateFormat.MMMMd(tag).format(date);
    }
  }
  ...
  switch (style) {
    ...
    case LumoDateStyle.dayMonth:
      return '${n(j.day)} ${jalaliMonthName(j.month, locale)}';
  }
}
