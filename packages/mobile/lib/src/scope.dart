...
/// The SAME set the gate falls back to, checked against ICU tag by tag.
/// `ku` is not here — bare `ku` is Kurmanji, Latin script, and `ckb` carries
/// Sorani — and `prs` (Dari) is. The `language-script` entries matter because
/// Azerbaijani, Punjabi and Uzbek are RTL only in Arabic script.
///
/// `test/direction_agreement_test.dart` reads the gate's own list out of
/// `packages/gate/src/index.ts` and fails if these two drift apart. The web
/// side asks `Intl.Locale.getTextInfo()` and keeps its list only for a
/// runtime without it; Dart has no equivalent, so here the list IS the answer.
const _rtlPrimary = {'ar', 'arc', 'az-arab', 'ckb', 'dv', 'fa', 'he', 'iw', 'khw', 'ks', 'nqo', 'pa-arab', 'prs', 'ps', 'rhg', 'sd', 'syr', 'ug', 'ur', 'uz-arab', 'yi'};

TextDirection directionOf(String locale) {
  final parts = locale.toLowerCase().split(RegExp('[-_]'));
  final primary = parts.first;
  // `az`, `pa` and `uz` are RTL only in Arabic script, so the script form has
  // to be looked up as well — matching on the primary subtag alone made those
  // entries unreachable, which is exactly the bug the web side had.
  final script = parts.length > 1 ? '$primary-${parts[1]}' : primary;
  return _rtlPrimary.contains(primary) || _rtlPrimary.contains(script)
      ? TextDirection.rtl
      : TextDirection.ltr;
}
