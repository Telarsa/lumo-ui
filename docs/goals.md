...
| 10 ◐ (G3: smoke runs a bundler AND a Node-ESM consumer; contract packages carry explicit extensions; adapters not started) | **G4 integrations**, G3 | 6 → 8 / 7 → 8 | Router adapter (`Link`/`MenuItem` href with Next and plain anchors), TanStack Query async-collection adapter, form adapter parity across all fields; a Vite consumer added to the smoke script beside Next. | Adapters documented with a worked example each; smoke runs both consumers. | +0.1 |
