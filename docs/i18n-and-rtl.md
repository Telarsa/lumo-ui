...
lang/dir on the root · no Latin digits in Persian/Arabic text · no Latin ARIA strings · every control named · IDREFs resolve · composite widgets have exactly one Tab stop · native calendar in dates · unique ids · native-script text and names · named roledescription · per-route Persian digit floors (auto-admitted for any route with 30+ native digits).
...
`data-lumo-latn` marks LTR islands (URLs, code, key caps) that must not mirror and are exempt from digit rules. `top-*` stays physical: the block axis does not mirror.
