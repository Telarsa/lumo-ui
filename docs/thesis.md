...
- **Copy-in distribution.** 111 components and 30 product blocks ship as source through a registry, so a consumer owns and edits the code that meets their validation and data. `@lumo-ui/core` (locale, direction, `LumoNode`, formatting, strings) and `@lumo-ui/base-ui-ssr` (first-byte compensations for the engine) are dependencies, because they are contracts you want maintained beneath you.
...
Telarsa's own React products first, and any React application that has to be right in Persian or another RTL language without a team of reviewers reading every screen. It aims to stand beside shadcn/ui, Mantine and the other established libraries as a contender for React applications that must be right in Persian or another RTL language — the criteria and the comparison are `docs/rubric.md`; English-first apps with no such requirement are better served by their own ecosystems. Lumo earns its weight only where the language rules do work.
