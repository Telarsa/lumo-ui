#!/usr/bin/env node
/**
 * example-projects/lumo-app-lynx/src/lumo/tokens.css is GENERATED (Lynx CSS: hex colours, px sizes) from packages/theme/src/tokens.css:
...
function colour(/** @type {string} */ resolved) {
  ...
  if (m) {
    const L = Number(m[1]);
    // Lynx CSS has no oklch(): the achromatic default is baked in; a brand regenerates with its knobs.
    if (m[2] === "{--lumo-ref-chroma-brand}") return oklchToHex(L, BRAND.chroma, BRAND.hue);
    if (m[2] === "{--lumo-ref-chroma-neutral}") return oklchToHex(L, BRAND.neutralChroma, BRAND.neutralHue);
  ...
  if (a) return oklchToHex(Number(a[1]), Number(a[2]), Number(a[3])) + Math.round(Number(a[4]) * 255).toString(16).padStart(2, "0");
...
const brandParts = brandArg === -1 ? [] : (process.argv[brandArg + 1] ?? "").split(",").map(Number);
const BRAND = { hue: brandParts[0] ?? 0, chroma: brandParts[1] ?? 0, neutralHue: brandParts[2] ?? 0, neutralChroma: brandParts[3] ?? 0 };
function scheme(/** @type {Map<string, string>} */ sys) {
  return COLOURS.map((name) => {
    ...
    return `  --lumo-sys-${name}: ${colour(resolve(raw, sys))};`;
  }).join("\n");
}
...
const out = `/* GENERATED by scripts/build-lynx-tokens.mjs (lumo-ui) from packages/theme/src/tokens.css — do not edit.
 * The same --lumo-sys-* semantic tokens as the web theme, resolved for Lynx CSS:
 * hex colours (Lynx has no oklch()), px sizes (rem × 16 at density 0.9). Two
 * schemes as classes on the Lumo root. Brand: regenerate with --brand h,c. */
.lumo-root {
${scheme(SYS_LIGHT)}
  --lumo-radius-sm: ${dp("var(--lumo-ref-radius-sm)")}px;
  --lumo-radius-md: ${dp("var(--lumo-ref-radius-md)")}px;
  --lumo-radius-lg: ${dp("var(--lumo-ref-radius-lg)")}px;
  --lumo-control-sm: ${dp("var(--lumo-ref-control-sm)")}px;
  --lumo-control-md: ${dp("var(--lumo-ref-control-md)")}px;
  --lumo-control-lg: ${dp("var(--lumo-ref-control-lg)")}px;
  --lumo-focus-width: ${parseInt(SYS_LIGHT.get("--lumo-sys-focus-width") ?? "2")}px;
}
.lumo-root.lumo-dark {
${scheme(SYS_DARK)}
}
...
if (process.argv.includes("--check")) {
  ...
  console.log(`  lynx-tokens: ${COLOURS.length} colours × 2 schemes checked`);
} else {
  await writeFile(OUT, out);
  console.log(`  lynx-tokens: wrote ${OUT} (${COLOURS.length} colours × 2 schemes)`);
}
