#!/usr/bin/env node
/**
 * gate:flutter — `flutter analyze` + `flutter test` for packages/mobile,
...
const run = (/** @type {string[]} */ args) => spawnSync(flutter, args, { cwd: PKG, stdio: "inherit", env: { ...process.env, PATH: `${process.env.PATH ?? ""}:/opt/homebrew/share/flutter/bin` } }).status ?? 1;
